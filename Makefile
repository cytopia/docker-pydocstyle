...
_test-version:
	...
	@if [ "$(VERSION)" = "latest" ]; then \
		...
		LATEST="$$( \
			curl -L -sS  https://github.com/PyCQA/pydocstyle/releases/ \
				| tac | tac \
				| grep -Eo "PyCQA/pydocstyle/releases/tag/v?[.0-9]+" \
				| head -1 \
				| sed 's/.*tag\///g' \
		)"; \
		...
		if ! docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version | grep -E "^$(VERSION)[.0-9]+$$"; then \
			docker run --rm --platform $(ARCH) $(IMAGE):$(DOCKER_TAG) --version; \
			echo "Failed"; \
			exit 1; \
		fi; \
